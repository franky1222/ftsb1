...
//        console.log("line---leave")
   ...
   var x=new Array()
    function c(){
      ...
      for(var i=0,j=1000;i<myArray.length-1;i=i+4,j++)
      {
       x2=myArray[i]*data2*yli;
       x1=myArray[i+1]*data1*xli;
       x4=(myArray[i+2]-myArray[i])*data2*yli;
       x3=(myArray[i+3]-myArray[i+1])*data1*xli;
       var rectangleView = "<div id='" +j + "' class='line' x='" + x1 + "' y='" + x2 + "' style='left: " + x1 + "px;top:" + x2 + "px;width:"+x3+"px;height:" + x4 + "px'><img draggable='false' class='change_size'><div class='glyphicon glyphicon-remove delete_line'></div></div>";
            $('.biaoDiv').append(rectangleView);
            isFirstMove = false;
              console.log(j + "first")
            $("#rightContentbody").append('<tr id="' + j + '"><td>'+(x4*yli).toFixed(2)+'</td><td>'+(x3*xli).toFixed(2)+'</td><td>' + (x1*xli).toFixed(2) + '</td><td>' + (x2*yli).toFixed(2) + '</td></tr>')
       }
    }
